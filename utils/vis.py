...
def vis_multi_class(imt, ant, pred, save_dir, n_class=3,
                    is_boundary=True):  # 输入N*H*W，输出路径/result/data_path/eval/fusion/vis
    # 拼接图片从左至右 原图、预测、金标准
...
def draw_img(img, seg, title=None, is_boundary=True, n_class=3):  # 原图上色
...
def gray2rgbimage(image):  # 单通道变三通道，方便后续的彩色图片拼接,单纯得就是将单通道变三通道
    a, b = image.shape    
