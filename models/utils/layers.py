...
class unetUp_edge(nn.Module):
    def __init__(self, in_size, out_size, is_deconv, n_concat=2):
        super(unetUp_edge, self).__init__()
        self.conv = unetConv2(in_size//2, out_size, is_batchnorm=True)
        if is_deconv:
    ...
    def forward(self, inputs0, *input):
        ...
        att = 1 - torch.sigmoid(outputs0)
        if input is not None:
            for i in range(len(input)):
                outputs0 = att * input[i]
...
class unetUp_nonlocal(nn.Module):
    def __init__(self, in_size, out_size, is_deconv, deep_s=False):
        super(unetUp_nonlocal, self).__init__()
        ...
        self.inter_channel = 3
        self.conv_k = nn.Sequential(nn.Conv2d(in_channels=in_size // 2, out_channels=self.inter_channel, kernel_size=1, stride=1,
                                padding=0, bias=False)
                                    )
                                    
                                    # 改动2
        self.conv_k2 = nn.Sequential(nn.Conv2d(in_channels=in_size // 2, out_channels=in_size//2, kernel_size=1, stride=1,
                                padding=0, bias=False)
                                    )
        self.conv_last = nn.Sequential(         # 改动1
            nn.Conv2d(in_channels=in_size // 2, out_channels=in_size // 2, kernel_size=1, stride=1, padding=0,
                      bias=False),
            nn.BatchNorm2d(in_size // 2),
            nn.ReLU()
        )

        self.conv_v = nn.Sequential(nn.Conv2d(in_channels=in_size // 2, out_channels=self.inter_channel, kernel_size=1, stride=1,
                                padding=0, bias=False)
                                    )
        self.deep_s = deep_s
        # self.alpha = nn.Parameter(torch.zeros(1))   # 这里做了修改
        # initialise the blocks
        for m in self.children():
    ...
    def forward(self, right, left):
        ...
        right = self.up(right)
        # print('fgahjsfgahsgfs')
        fus = right + left
        b, c, h, w = right.size()
        # (N,3,h,w) 这个输出希望能做深监督的输出
        # x_guide_map = self.conv_k(right)
        x_guide_map = self.conv_k(fus)   # 注意这里做了一个改动，7/24的17：55
        # n,hw,3
        x_guide_map_k = x_guide_map.view(b, -1, h * w).permute(0, 2, 1).contiguous()
        # n,c,hw
        right_k = self.conv_k2(fus).view(b, -1, h * w)
        # print(x_guide_map_k.shape, fus.shape)

        # n,c,3 表示每个通道是由c个通道的共同表征。
        sim_out = F.softmax(torch.matmul(right_k, x_guide_map_k), dim=1)

        # left 的通道也变成这样,(n,3,hw)
        x_guided_map = self.conv_v(left).view(b, -1, h * w).contiguous()
        out = torch.matmul(sim_out, x_guided_map)
        out = out.view(b, -1, h, w)
        
        # out = self.conv_last(out)
        # print(out.shape,left.shape)
        out = out + left

        output = torch.cat((right, out), dim=1)
        output = self.conv(output)
        # print(output)
        if self.deep_s:
            return x_guide_map, output
        else:
            return output