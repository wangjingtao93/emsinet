...
from models.nets.model_part.res2net_v1b import res2net50_v1b_26w_4s
import os
...
class PraNet(nn.Module):
    ...
    def __init__(self, channel=32):
        ...
        self.ra2_conv4 = BasicConv2d(64, 3, kernel_size=3, padding=1)

    def forward(self, x):

        x = self.resnet.conv1(x) # torch.Size([2, 64, 176, 176])
        x = self.resnet.bn1(x)
        ...
        ra5_feat = self.agg1(x4_rfb, x3_rfb, x2_rfb) # global map Sg

        lateral_map_5 = F.interpolate(ra5_feat, scale_factor=8, mode='bilinear')    # NOTES: Sup-1 (bs, 1, 44, 44) -> (bs, 1, 352, 352)

        # ---- reverse attention branch_4 ----
        crop_4 = F.interpolate(ra5_feat, scale_factor=0.25, mode='bilinear') # Down-sample
        ...
        crop_3 = F.interpolate(x, scale_factor=2, mode='bilinear') # 上采样
