...
class UNet_DAFM(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # final_1 =  F.log_softmax(final_1,dim=1)
        return final_1




# class UNet_nonlocal_skip(nn.Module):

#     def __init__(self, in_channels=1, n_classes=4, feature_scale=4, is_deconv=True, is_batchnorm=True):
#         super(UNet_nonlocal_skip, self).__init__()
#         self.is_deconv = is_deconv
#         self.in_channels = in_channels
#         self.is_batchnorm = is_batchnorm
#         self.feature_scale = feature_scale
...
#         # self.model_concat = nn.ModuleList(Nonlocal_channel(filter[3], deep_s=False),
#         #                                   Nonlocal_channel(filter[2], deep_s=False),
#         #                                   Nonlocal_channel(filter[1], deep_s=False),
#         #                                   Nonlocal_channel(filter[0], deep_s=False),
#         #                                   )
#         # upsampling
#         self.up_concat4 = unetUp_nonlocal(filters[4], filters[3], self.is_deconv)
#         self.up_concat3 = unetUp_nonlocal(filters[3], filters[2], self.is_deconv)
#         self.up_concat2 = unetUp_nonlocal(filters[2], filters[1], self.is_deconv)
#         self.up_concat1 = unetUp_nonlocal(filters[1], filters[0], self.is_deconv)
...
class UNet_nonlocal_skip_deepsupervision(nn.Module):
    ...
    def __init__(self, in_channels=1, n_classes=4, feature_scale=4, is_deconv=True, is_batchnorm=True):
        ...
        #                                   )
        # upsampling
        self.up_concat4 = unetUp_nonlocal(filters[4], filters[3], self.is_deconv,deep_s=True)
        self.up_concat3 = unetUp_nonlocal(filters[3], filters[2], self.is_deconv,deep_s=True)
        self.up_concat2 = unetUp_nonlocal(filters[2], filters[1], self.is_deconv,deep_s=True)
        self.up_concat1 = unetUp_nonlocal(filters[1], filters[0], self.is_deconv,deep_s=True)
    ...
    def forward(self, inputs):
        ...
        # 插入多尺度的模块
        # center = self.muiti_p(center) 
        out4,up4 = self.up_concat4(center, conv4)  # 128*64*128
        out3,up3 = self.up_concat3(up4, conv3)  # 64*128*256
        out2,up2 = self.up_concat2(up3, conv2)  # 32*256*512
        out1,up1 = self.up_concat1(up2, conv1)  # 16*512*1024

        final_1 = self.final_1(up1)

        # final_1 =  F.log_softmax(final_1,dim=1)
        return [out4,out3,out2,out1], final_1
...
if __name__ == '__main__':
    ...
    net = UNet_DAFM(in_channels=1, n_classes=4, is_deconv=True).cuda()
    print(net)
