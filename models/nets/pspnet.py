...
import models.nets.resnet as models
...
class PPM(nn.Module):
    def __init__(self, in_dim, reduction_dim, bins, BatchNorm):
        ...
        self.features = []
        for bin in bins:
            # self.features是一个列表，列表中的每一项是多个卷积层
            # ，用来将输入的特征图自适应平均池化到指定大小，然后用1*1的卷积降低维度
...
class PSPNet(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        # 对输入图片的尺寸有限制，（473-1）%8=0
        h = x_size[2]
        # （(x_size[2] - 1) / 8 * self.zoom_factor + 1）=（472/8）×1+1=60
        w = x_size[3]
        x = self.layer0(x)  # size为(4,128,119,119) 下采样了4倍
        # ，因为resnet的输入就是下采样，然后layer1会用Maxpool下采样
        x = self.layer1(x)  # layer1中只有常规卷积，没有空洞卷积和下采样，输出为(4,256,119,119)
        x = self.layer2(x)  # layer2用卷积层stride=（2,2）下采样一次，输出为（4,612,60,60）
        x_tmp = self.layer3(x)  # 输出为（4,1024,60,60），没有下
        # 采样，使用了6个（2*2）的空洞卷积来扩大感受野
        x = self.layer4(x_tmp)  # 使用了3个（4*4）的空洞卷积来扩大感受野，输出为（4,2048,60,60）
        if self.use_ppm:
            x = self.ppm(x)  # 输入是（4,2048,60,60）,然后使用自适应全
        # 局平均池化分别得到size为1*1,2*2,3*3,6*6的feature map,这些featuremap都转化为512通道,然后将x与所有的feature map做concat为4096通道
        x = self.cls(x)
