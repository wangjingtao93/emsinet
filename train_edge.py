...
from  data.edge_dataloader import Slit_loader
...
def val(args, model, dataloader, epoch):
    ...
    with torch.no_grad():  # 适用于推断阶段，不需要反向传播
        ...
        for i, (data, labels, edge) in enumerate(tbar):  # i=300/batch_size=3
            # tbar.update()
            ...
            predicts, _ = model(data) 

            # _ =  torch.argmax(_, dim=1)[0]
            # label_out = label[0].squeeze().cpu().numpy() * 125
            # a = (edge[0].squeeze().numpy()*255).astype('uint8')
            # # print(a.shape,_.shape, label_out.shape)
            # cv2.imwrite('img2.png', (_.cpu().numpy()*255).astype('uint8'))
            # cv2.imwrite('gt2.png', label_out.astype('uint8'))
            # cv2.imwrite('gt_edge2.png', a)
...
def train(args, model, optimizer, criterion, dataloader_train, dataloader_val, writer, k_fold,current_time):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (data, label, edge) in enumerate(dataloader_train):  # i= 14*128/4= 448
            ...
            main_out, edge_out = model(data)  # 前向传播
            ...
            loss_edge = criterion[2](edge_out, edge[:,0])
            # print(loss_edge)
            # loss_bce = criterion[2](torch.sigmoid(main_out), utils.loss.mask_to_onehot(label, palette).float())
            loss = loss_cel + loss_el + loss_edge
